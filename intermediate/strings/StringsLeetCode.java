...
public class StringsLeetCode {
    ...
    // Fastest Solution - 1 ms(2 pointer approach)

    public boolean areSentencesSimilarFastest(String sentence1, String sentence2) {
        String[] words1 = sentence1.split(" ");
        String[] words2 = sentence2.split(" ");
        
        int i = 0; // pointer for prefix
        int j = 0; // pointer for suffix
        ...
        // If the remaining words in the longer sentence can be removed
    ...
    // Fastest Solution - 9 ms

    public boolean checkInclusionFastest(String s1, String s2) {
        if (s1.length() > s2.length())
            return false;
        int[] s1arr = new int[26];
        int[] s2arr = new int[26];
    ...
    // Brute force solution - O(n^2) - 220 ms

    public String shortestPalindrome(String s) {
        int n = s.length();
         String rev = new StringBuilder(s).reverse().toString();
         
         // Try each prefix of the original string as a palindrome
         for (int i = n; i >= 0; i--) {
             if (s.substring(0, i).equals(rev.substring(n - i))) {
     ...
     // Fastest Solution - 5 ms

     public String shortestPalindromeFastest(String s) {
        int length = s.length();
        if (length == 0) {
            return s;
        }
